...
export class SetLeaderboardCommandHandler implements ICommandHandler<SetLeaderboardCommand>{
  ...
  async execute() {
    ...
    const leaderboard: ProfileLeaderboardDto[] =  await this.profileLeaderboardDtoRepository.getLeaderboard();
    ...
    const profilesPromise = await this.httpService.get(url + '/api/profile/leaderboard').toPromise();
